...
@click.option('--opt', type=str, help='Workflow-specific optional arguments.')
@click.pass_context
def odf_track(ctx, workflow, odf, seed, algorithm, angle, angle_range, min_length, opt):
    ...
    param = pe.Node(
        ...
        name="param_node")
    if angle is not None or algorithm is not None or min_length is not None:
        param.iterables = []
    ...
    if min_length is not None:
        mls = min_length.split(',')
        mls = [int(l) for l in mls if l.isdigit()]
        param.iterables.append(('min_length', mls))
    wf = ctx.obj['workflow']
    if seed is not None:
    ...
    wf.connect([(param, wf_sub, [("angle", "inputnode.angle")]),
                ...
                (param, wf_sub, [("min_length", "inputnode.min_length")]),
...
def tck_filter(ctx, workflow, tck, odf, ensemble):
    ...
    try:
        if workflow is not None or not ensemble:
            wf_mod = import_module('.workflows.' + workflow, package='trampolino')
        ...
        click.echo('No workflow selected (maybe --ensemble is missing?).')
        sys.exit(1)
    wf = ctx.obj['workflow']
